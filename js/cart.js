...
$.get("data/cart/list.php").then(result=>{
  console.log(result)
  if (result.code === 300) {
    alertMsg('您尚未登录！');
      ...
      $.each(result.data, function (i, p) {
        html += `
            <div class="imfor">
              ...
              <div class="pudc">
                <div class="pudc_information" id="${p.iid}">
                  <a href="productdetail.html?pid=${p.pid}"><img src="${p.sm}" class="lf"/></a>
                  <input type="hidden" name="" value="">
                          <span class="des lf">
                              <a href="productdetail.html?pid=${p.pid}">${p.title}</a>
                                <input type="hidden" name="" value="">
                          </span>
                  <p class="col lf"><span>规格：</span><span class="color_des">${p.spec}  <input type="hidden" name=""
                                                                                        value=""></span></p>
                </div>
              </div>
              <div class="pices">
                <p class="pices_des"></p>
                <p class="pices_information"><b>￥</b><span>${p.nprice}  <input type="hidden" name="" value=""></span></p>
              </div>
              <div class="num"><span class="reduc">&nbsp;-&nbsp;</span><input type="text" value="${p.count}"><span class="add">&nbsp;+&nbsp;</span>
              </div>
              <div class="totle">
                <span>￥</span>
                <span class="totle_information">${p.nprice * p.count}</span>
              </div>
              <div class="del">
